...
def main(args, res_dir):
    ...
    if (globcount<=globcount_thres):
        ...
        with open(('./content/model.txt'), 'w') as f:
            f.write(str(MODELS_TO_RUN[0]))
        push_github("push_model_file")
        _y_train_raw = task(_pt, reference_path, globcount, run_eval=True)
    ...
    for i in tqdm(range(num_iter), total=num_iter):
        ...
        if (globcount <= globcount_thres):
            ...
            with open(('./content/model.txt'), 'w') as f:
                f.write(str(MODELS_TO_RUN[i+1]))
            push_github("push_model_file")

            y_new = task(_pt, reference_path, globcount, run_eval=True)
        globcount += 1
